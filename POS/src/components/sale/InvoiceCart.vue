						...
						<div class="flex-1 flex items-center gap-1.5 bg-white border border-gray-200 rounded-xl p-1.5 shadow-sm min-w-0">
							...
							<div class="flex items-center gap-0.5 flex-shrink-0" @click.stop>
								...
								<button
									...
									:title="__('View customer account / dues')"
								>
									<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
				...
				<div v-else>
					<div class="flex gap-1.5">
						...
						<div class="relative flex-1">
							...
							<!-- F8 shortcut hint (hidden while typing to make room for clear) -->
							<div
								v-if="!customerSearch"
								class="absolute inset-y-0 end-0 pe-2.5 hidden md:flex items-center pointer-events-none"
							>
								<KbdHint keys="F8" />
							</div>
						...
						<div class="flex-1 min-w-0 flex flex-col justify-center">
							...
							>
								...
								>
									...
									{{ __('Cost') }}: {{ formatCurrency((item.valuation_rate || 0) * (item.conversion_factor || 1)) }}
								</span>
							</div>
						</div>
					</div>
		...
		<div class="p-1.5 sm:p-2 bg-white border-t border-gray-200">
			...
			<div class="flex gap-1.5">
				...
				>
					...
					<KbdHint keys="F9" variant="overlay" class="ms-1.5" />
				</button>
...
const emit = defineEmits([
	...
	"view-customer-dues", // (customer) - Open customer account/dues dialog
])
...
function handleKeydown(event) {
	if (event.key === "Escape") {
		...
		customerSearchFocused.value = false
		// Restore the customer that was deselected by focusCustomerSearch (F8)
...
defineExpose({
	async focusCustomerSearch() {
		// The search input only renders when no customer is selected, so a
		// selected customer must be temporarily deselected to reveal it.
		// previousCustomer makes outside-click/Escape restore the selection.
		if (props.customer) {
