	...
	<Dialog v-model="isOpen" :options="{ title: dialogTitle, size: 'md' }">
		<template #body-content>
			<div class="py-4">
				...
				<div
					v-else-if="mode === 'variant' && options.length > 0"
								...
								<p
									...
									:class="
										(matchedVariant.stock ??
											matchedVariant.data?.actual_qty ??
											0) > 0
											? 'text-green-600'
											: 'text-red-600'
									"
				...
				<div v-else-if="mode === 'uom' && options.length > 0" class="flex flex-col gap-4">
					...
					<div>
						...
						<div
							class="grid gap-2"
							:class="options.length <= 2 ? 'grid-cols-2' : 'grid-cols-3'"
						>
							<button
					...
					<div>
						...
						<div
							class="w-full h-10 border border-gray-300 rounded-lg bg-white flex items-center overflow-hidden"
						>
							<button
								type="button"
					...
					<div class="bg-blue-50 rounded-xl p-4 flex items-center justify-between">
						<div>
							...
							<p class="text-xs text-gray-500">
								{{ quantity }} ×
								{{ formatCurrency(selectedOption?.rate || options[0]?.rate || 0) }}
							</p>
						</div>
					...
					<p
						v-if="stockWarning"
						class="text-xs text-orange-600 flex items-center justify-center gap-1 bg-orange-50 rounded-lg p-2"
					>
				...
				<div v-else class="text-center py-8">
					<div
						class="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-orange-100 mb-3"
					>
						<svg
					...
					<div v-if="mode === 'variant'" class="mt-4">
						...
						<ol
							class="text-xs text-gray-600 text-start max-w-xs mx-auto flex flex-col gap-1"
						>
							<TranslatedHTML
								:tag="'li'"
								:inner="
