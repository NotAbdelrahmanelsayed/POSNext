...
export const usePOSCartStore = defineStore("posCart", () => {
	...
	function setIsCreditSale(value) {
	...
	async function submitInvoice(options = {}) {
		...
		const result = await baseSubmitInvoice(
			...
			Boolean(options.isCreditSale || isCreditSale.value),
		)
		// Reset write-off amount and credit sale flag after successful submission
		if (result) {
	...
	function processFreeItems(freeItems) {
		...
		for (const freeItem of freeItems) {
			...
			invoiceItems.value.push({
				...
				item_name:
					freeItem.item_name || cartItem?.item_name || freeItem.item_code,
				rate: 0,
				price_list_rate: 0,
				quantity: freeQty,
				discount_amount: 0,
