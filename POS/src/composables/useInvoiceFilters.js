...
export function useInvoiceFilters(invoices) {
	...
	const uniqueProducts = computed(() => {
		...
		return Array.from(productsMap.values()).sort((a, b) => a.label.localeCompare(b.label));
	});
	...
	function getCustomerName(customerValue) {
		const customer = uniqueCustomers.value.find((c) => c.value === customerValue);
	...
	const filterStats = computed(() => {
		return {
			...
			percentage: invoices.value?.length
				? Math.round((filteredInvoices.value.length / invoices.value.length) * 100)
				: 0,
		};
	});
