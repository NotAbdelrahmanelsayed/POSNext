...
export function useSearchInput({ itemStore, onItemFound, showWarning, isAnyDialogOpen }) {
	...
	function handleKeyDown(event) {
		if (event.key === "Enter") {
			...
			// If search results are visible, add the first one directly. Guarded
			// to non-scanner mode: in scanner mode filteredItems can fall back to
			// the wrong cached item, so always do a barcode lookup there.
			const firstResult = itemStore.filteredItems?.[0];
			if (!scannerEnabled.value && firstResult && itemStore.searchTerm) {
			...
			// No usable search result (or scanner mode) → barcode lookup.
			// Clear input immediately so the next scan starts clean.
			itemStore.clearSearch();
