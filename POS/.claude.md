...
**Composables & Stores (.js files inside `src/`)** → **Use `call` from `@/utils/apiWrapper`**

```javascript
// ✅ CORRECT - Composable or store making a one-off API call
import { call } from '@/utils/apiWrapper'

const result = await call('pos_next.api.items.get_item_details', {
    item_code: 'ITEM-001',
    pos_profile: 'Profile Name',
})
// result is the unwrapped message — no need for result.message
```

> `@/utils/apiWrapper` wraps frappe-ui's `call` with CSRF auto-refresh. Always import from here, not directly from `frappe-ui`, and **not** `window.frappe.call` (which is Frappe Desktop and may be unavailable in the standalone POS SPA).
...
**Solution:** In Vue components, use `createResource` which handles API availability automatically. In composables/stores, use `call` from `@/utils/apiWrapper` — do NOT use `window.frappe.call` in the standalone POS SPA.
...
### Issue: `Item.valuation_rate` is 0 for most items
**Root cause:** ERPNext only populates `Item.valuation_rate` for the "Standard" valuation method. Most items use FIFO or Moving Average, where the actual cost is in `Bin.valuation_rate` per warehouse, or in `Item Price` (buying=1).

**Solution:** When displaying or reading a cost/buying price for an item, use this priority:
1. `Item Price` where `buying = 1` (Standard Buying price list)
2. `Bin.valuation_rate` for the item's warehouse
3. `Item.valuation_rate` as last resort

See `get_item_detail()` and `get_items()` in `pos_next/api/items.py` for the implemented fallback logic.

### Issue: Cart item `valuation_rate` is 0 despite correct backend data
**Root cause:** Item search uses cache-first (IndexedDB). If the user clicks before server results arrive, the old cached item (without `valuation_rate`) is used.

**Solution:** `useInvoice.addItem()` fires a background call to `get_item_details` when `valuation_rate = 0`. It sets the result on the **reactive proxy** (`invoiceItems.value[lastIndex]`), not the original plain object — this is critical for Vue to detect the change and re-render.

### Issue: Python changes don't take effect after `bench clear-cache`
**Root cause:** Gunicorn runs with `--preload`. Workers fork from a preloaded master, keeping old code in memory.

**Solution:**
```bash
kill -HUP $(pgrep -f "gunicorn.*frappe" | head -1)
```
This sends SIGHUP to the master process, triggering a graceful worker reload with the new code.
